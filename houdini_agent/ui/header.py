...
class _ReasoningEffortComboBox(QtWidgets.QComboBox):
    """Compact pill combo with a stable drawn arrow."""
    ...
    def paintEvent(self, event):  # noqa: N802
        ...
        bg = QtGui.QColor(224, 184, 122, 38 if (hovered or focused) else 28)
        border = QtGui.QColor(224, 184, 122, 140 if (hovered or focused) else 90)
        text = QtGui.QColor("#f0d2a0" if (hovered or focused) else "#e0b87a")
        arrow = QtGui.QColor("#c8aa72" if (hovered or focused) else "#b99762")
        ...
        text_rect = self.rect().adjusted(9, 0, -18, 0)
        painter.setPen(text)
        ...
        points = QtGui.QPolygonF([
            QtCore.QPointF(cx - 3, cy - 2),
            QtCore.QPointF(cx + 3, cy - 2),
            QtCore.QPointF(cx, cy + 2),
        ])
...
class HeaderMixin:
    ...
    def _build_header(self) -> QtWidgets.QWidget:
        ...
        self._register_custom_model_metadata()
        ...
        self.key_status.setFixedHeight(22)
        self.key_status.setMaximumWidth(104)
        self.key_status.setMinimumWidth(72)
        self.key_status.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        ...
        self.web_check.setFixedHeight(22)
        self.web_check.setMinimumWidth(38)
        self.web_check.setCursor(QtCore.Qt.PointingHandCursor)
        ...
        self.think_check.setFixedHeight(22)
        self.think_check.setMinimumWidth(46)
        self.think_check.setCursor(QtCore.Qt.PointingHandCursor)
        row.addWidget(self.think_check)
        
        self.reasoning_effort_combo = _ReasoningEffortComboBox()
        self.reasoning_effort_combo.setObjectName("reasoningEffortCombo")
        self.reasoning_effort_combo.setToolTip("Custom GPT-5 reasoning effort")
        self.reasoning_effort_combo.addItem("Low", "low")
        self.reasoning_effort_combo.addItem("Med", "medium")
        self.reasoning_effort_combo.addItem("High", "high")
        self.reasoning_effort_combo.addItem("xhigh", "xhigh")
        self.reasoning_effort_combo.setCurrentIndex(2)
        self.reasoning_effort_combo.setFixedSize(64, 22)
        self.reasoning_effort_combo.setCursor(QtCore.Qt.PointingHandCursor)
        self.reasoning_effort_combo.view().setMinimumWidth(76)
        self.reasoning_effort_combo.view().setUniformItemSizes(True)
        self.reasoning_effort_combo.setVisible(False)
        row.addWidget(self.reasoning_effort_combo)
    ...
    def _register_custom_model_metadata(self):
        """将 Custom Provider 模型注册到上下文和能力表。"""
        cc = getattr(self, '_custom_provider_config', {}) or {}
        models = cc.get('models') or []
        if not models:
            return
        context_limit = int(cc.get('context_limit') or 128000)
        if hasattr(self, '_model_context_limits'):
            for model in models:
                self._model_context_limits[model] = context_limit
        if hasattr(self, '_model_features'):
            for model in models:
                self._model_features[model] = {
                    'supports_prompt_caching': True,
                    'supports_vision': bool(cc.get('supports_vision', False)),
                }
    ...
    def _on_provider_changed_custom_visibility(self):
        ...
        self.reasoning_effort_combo.setVisible(is_custom)
